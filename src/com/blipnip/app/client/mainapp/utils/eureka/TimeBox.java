...
import com.blipnip.app.client.mainapp.utils.eureka.ResourcesBundle;
...
public class TimeBox extends Composite implements HasValueChangeHandlers<Long>, HasValue<Long>, HasBlurHandlers, Focusable 
{
...
private class DummyDate extends Object
{
	...
	public long getTime()
	{
		long duration = hours*ONE_HOUR + minutes*ONE_MINUTE;
		System.out.println("@TimeBox, @getTime, duration specified is: "+duration+" (in milliseconds)");
	...
	//Changed to DummyDate from Date, because methods used are depracated.
	private final DummyDate timeValue;
	
	public ValueTextBox getHoursBox()
	{
		return this.hoursBox;
	}
	
	public ValueTextBox getMinutesBox()
	{
		return this.minutesBox;
	}
	...
	public TimeBox(Date time, TIME_PRECISION precision, boolean useAMPM) {
		...
		timeValue = new DummyDate();
		this.useAMPM = useAMPM;

		container = new VerticalPanel();

		int hour = timeValue.getHours();        //time.getHours();
		int minutes = timeValue.getMinutes();//time.getMinutes();
		...
		minutesBox.addKeyUpHandler(new KeyUpHandler() {
			...
			public void onKeyUp(KeyUpEvent event) {
				...
				if (!stepKey && (oldMinutes != minutes)) {
					...
					DummyDate tmpValue = new DummyDate();
					tmpValue.setHours(timeValue.getHours());
